...
本記事では、情報検索（Information Retrieval, IR）システムの性能を測る代表的な評価指標を、定義式とその意味を中心に数理的に整理する。集合ベースの指標から順位（ランキング）を考慮した指標、複数クエリにわたる集約、そしてオフライン評価の前提までを順に扱う。
...
- **二値関連度**: $$\mathrm{rel}(d) \in \{0, 1\}$$。関連なら 1、非関連なら 0。
- **段階的関連度**: $$\mathrm{rel}(d) \in \{0, 1, 2, \ldots, g_{\max}\}$$。例: 0=非関連, 1=やや関連, 2=関連, 3=完全一致。
