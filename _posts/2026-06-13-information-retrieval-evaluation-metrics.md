...
> **記法のまとめ:** $$\lvert \cdot \rvert$$ は集合の要素数、$$\mathbf{1}[\cdot]$$ は条件が真なら 1・偽なら 0 を返す指示関数とする。$$\mathrm{rel}_i \equiv \mathrm{rel}(d_{(i)})$$ は第 $$i$$ 位文書の関連度を表す。
...
ユーザーが上位から順に閲覧し、十分満足した時点で停止する、というカスケードモデルに基づく。位置 $$i$$ の文書で満足する確率を $$R_i = \dfrac{2^{\mathrm{rel}_i} - 1}{2^{g_{\max}}}$$ とすると、ERR は次式で与えられる。

$$
\mathrm{ERR} = \sum_{i=1}^{n} \frac{R_i}{i} \prod_{j=1}^{i-1} (1 - R_j)
$$

$$\prod_{j=1}^{i-1}(1 - R_j)$$ は「順位 $$i$$ より上で誰も満足しなかった確率」。MRR を段階的関連度へ一般化したものに相当する。
{:.note}
